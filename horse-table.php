    ...
    <body>
        ...
        <table>
            ...
            <?php 
                ...
                $query = "SELECT id_no, breed, color, gender FROM horses";
                $results = $conn->query($query);

                if ($results->num_rows > 0){
                    while ($row = $results->fetch_assoc()){
                        echo "<tr><td>" . $row["id_no"] . "</td><td>" .  $row["breed"] . "</td><td>" . $row["color"] . "</td><td>" .  $row["gender"] . "</td><td> <div class= 'btn-group'> <button type = 'button' class = 'btn btn-danger deletebtn'>Delete </div> </td></tr>";
                    }
                }else{
                    echo "No Results";
                }
